    ...
    try {
        ...
        if (data.email === '') {
            ...
            if (result.length === 0) {
                ...
                try {
                    const apiUrl = new URL("/api/owner/send", request.nextUrl.origin);
            ...
            if (result[0].length === 1) {
                ...
                const updateVerificationCode = `update owner set verificationCode = ? where email = ? and verificationCode = ?`;
                ...
                const baseUrl = new URL("/api/owner/send/welcome", request.nextUrl.origin);
