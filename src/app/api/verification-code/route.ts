...
export async function POST(request: NextRequest) {
    try {
        ...
        if (data.email === '') {
            ...
            const conn = await connection;
            const result: [any[], any] = await conn.query(query, [data.email, data.verificationCode]);
            ...
            if (result[0].length === 1) {
                ...
                const updateVerificationCode = `update user set verificationCode = ? where email = ? and verificationCode = ?`;
                ...
                const baseUrl = new URL("/api/send/welcome", request.nextUrl.origin);
